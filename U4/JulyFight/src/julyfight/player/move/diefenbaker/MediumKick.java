 ...
 * @author Darian
 */
public class MediumKick extends Move {

    private Animation animation;
    
    public MediumKick(Game game, Player player) {
        super(game, player);
        inputs = new Input[]{Input.MK};
    }
    ...
    public void execute() {
        ...
        player.renderer.setAnimation("MediumKick", 0.2);//Set animation
    }
    
    @Override
    public void init(GameContainer gc) {
        try {
            Image punch = new Image("julyfight/assets/diefenbaker/kick2.png");//Load image for animation
            animation = new Animation();//Initialise animation
            animation.addFrame(punch, 1);//Add frame
            player.renderer.addAnimation("MediumKick", animation);//Add to renderer
